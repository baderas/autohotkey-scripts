...
{
    Run *runas C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -command "$Host.UI.RawUI.WindowTitle = \"Running Chocolatey Update\"; choco -y upgrade all; Write-Host \"Press any key to exit ...\";$x = $host.UI.RawUI.ReadKey(\"NoEcho`,`IncludeKeyDown\");",,, process_id
    ;Run C:\Program Files\ConEmu\ConEmu64.exe -Title "Running Chocolatey Update" -run powershell.exe -new_console:a -command "$Host.UI.RawUI.WindowTitle = \"Running Chocolatey Update\"; choco -y upgrade all; Write-Host \"Press any key to exit ...\";$x = $host.UI.RawUI.ReadKey(\"NoEcho`,`IncludeKeyDown\");",,, process_id
